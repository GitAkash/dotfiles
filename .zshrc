...
fuzzy-xdg-open() {
  ...
  # If the MIME type is a terminal-based application (e.g., shell script, text, etc.)
  if [[ "$mime_type" =~ ^text/ || "$mime_type" == "application/x-shellscript" ]]; then
    # Run the terminal-based application in the current terminal
    if [[ -x "$output" ]]; then
      "$output"  # Execute the script or application directly
    else
      # If it's a text file, open it in `vim` (or any editor of your choice)
      hx "$output"  # You can replace vim with nano or any other editor
    fi
  else
    # Open the file using xdg-open in the background and suppress all output
  ...
  # Reset the prompt to ensure the terminal is interactive
  zle reset-prompt
}
